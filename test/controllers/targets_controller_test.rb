...
class TargetsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create target" do
    assert_difference('Target.count') do
      post targets_url, params: { target: { icon: @target.icon, name: @target.name, url: @target.url } }
    end
  ...
  test "should update target" do
    patch target_url(@target), params: { target: { icon: @target.icon, name: @target.name, url: @target.url } }
